...
def list_all():
    ...
    print(f"\n{'=' * 60}")
    print("Mimir Artifacts")
    print(f"{'=' * 60}\n")

    for artifact_id, info in artifacts.items():
        stale_marker = " [STALE]" if info.get("stale") else ""
        print(f"  {artifact_id}{stale_marker}")
        print(f"    Version:    {info.get('version', '1.0.0')}")
        print(f"    Dependencies: {info.get('dependency_count', 0)} files")
        print(f"    Last built: {info.get('last_built', 'unknown')}")
        print()
