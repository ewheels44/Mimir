...
if [ -z "$OPENSPACE_HOST_SKILL_DIRS" ]; then
    export OPENSPACE_HOST_SKILL_DIRS="/Users/ethanwheeler/Documents/Mimir/skills"
    log "Set default OPENSPACE_HOST_SKILL_DIRS: $OPENSPACE_HOST_SKILL_DIRS"
fi

if [ -z "$OPENSPACE_WORKSPACE" ]; then
    export OPENSPACE_WORKSPACE="/Users/ethanwheeler/Documents/OpenSpace"
    log "Set default OPENSPACE_WORKSPACE: $OPENSPACE_WORKSPACE"
fi

# Run the OpenSpace MCP server
cd "$SCRIPT_DIR"
exec "/Users/ethanwheeler/Documents/OpenSpace/.venv/bin/openspace-mcp" "$@"
